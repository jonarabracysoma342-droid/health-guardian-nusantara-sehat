...
function showGuruFormScreen() {
  ...
  // Always reset to login tab state when opening guru form
  if (viewResetPass) viewResetPass.classList.add('hidden');
...
export function initModals() {
  ...
  // Touch Lock for modal background on mobile
  ...
  // --- USER ACCESS SELECTION (SISWA VS GURU AUTHENTICATION) ---
  ...
  // Mode Selection Card Clicks (Tampilkan HANYA form yang dipilih)
  if (cardSiswa) {
  ...
  // Back Button Handlers (&leftarrow; Ganti Peran)
  ...
  // Submit Siswa Profile (Nama, Kelas, Sekolah - Tanpa Password)
  if (btnSubmitSiswa) {
  ...
  // --- GURU LUPA / RESET PASSWORD HANDLER ---
  ...
  // Menu navigation tiles click handler
  ...
  // Portal Kuis Tab Switching (Kuis Mandiri vs Kuis Kelas)
    ...
    if (container) {
      ...
      container.innerHTML = `
        <div style="display: flex; flex-direction: column; gap: 20px;">
          ...
          <!-- STATUS KELOMPOK SAAT INI / INPUT PIN -->
          ${currentGroup ? `
          ...
          <!-- PAPAN PERINGKAT SKOR KELOMPOK (LEADERBOARD TIM) -->
      ...
      const processJoin = async (code, pass) => {
        if (!code || !pass) {
          toast.error('Mohon isi Kode Kelas dan Kode Sandi Kelas.', {
            position: 'top-center',
          });
          return;
        }
        ...
        if (res.success) {
          ...
          gameState.saveState();
          toast.success(`Berhasil bergabung dengan ${data.player.group.name}! 🎉`, {
            position: 'top-center',
          });
...
export function showHistoryInfo(type) {
  ...
  if (type === 'stovia') {
    modalTitle.innerHTML = '<span class="icon">📜</span> Sejarah Pendirian STOVIA & Transformasi Medis';
    modalBody.innerHTML = `
...
// --- INTERACTIVE QUIZ ENGINE (20 SOAL EVALUASI KOMPETENSI) ---
const quizQuestions = [
  {
    id: 1,
...
let userAnswers = {};
let quizTimerSeconds = 1200; // 20 Menit
let quizTimerInterval = null;

export function initQuizEngine() {
  currentQuizIndex = 0;
  userAnswers = {};
  quizTimerSeconds = 1200; // 20 Menit untuk 20 Soal
        ...
        <div style="padding: 24px; overflow-y: auto; flex: 1; display: flex; flex-direction: column; gap: 20px;">
          ...
          <div style="background: #ffffff; border: 1.5px solid #e2e8f0; border-radius: 20px; padding: 24px; box-shadow: 0 4px 16px rgba(15,23,42,0.04);">
            ...
            <div id="quiz-question-container" style="font-family: var(--font-heading); font-size: 1.15rem; font-weight: 700; color: #0f172a; line-height: 1.5; margin-bottom: 24px;">
              <!-- Pertanyaan dimasukkan via JS -->
            </div>

            <!-- OPTIONS GRID (4 PILIHAN) -->
            <div id="quiz-options-container" style="display: grid; grid-template-columns: 1fr 1fr; gap: 14px; margin-bottom: 24px;">
              <!-- Options A, B, C, D dimasukkan via JS -->
            </div>

            <!-- NAVIGASI NOMOR SOAL (1-20) -->
            <div style="border-top: 1px solid #f1f5f9; padding-top: 20px;">
              ...
              <div id="quiz-nav-buttons" style="display: flex; justify-content: center; align-items: center; gap: 8px; flex-wrap: wrap;">
                <!-- Tombol 1-20 dimasukkan via JS -->
              </div>
            </div>

            <!-- NAVIGATION CONTROL BUTTONS (PREV / NEXT) -->
          ...
          <!-- BOTTOM WIDGETS GRID (TIPS & TIMER) -->
...
function finishQuiz() {
  ...
  // RENDER GOOGLE FORMS STYLE RESULT & REVIEW SCREEN
  const modalCard = document.querySelector('#modal-kuis .modal-card');
  if (!modalCard) return;
  ...
  modalCard.innerHTML = `
    ...
    <!-- REVIEW LIST BODY (GOOGLE FORMS STYLE DETAILED FEEDBACK) -->
